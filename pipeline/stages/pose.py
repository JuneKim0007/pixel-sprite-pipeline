...
def render_entries(ctx: Context, entries: list[dict], outdir: Path) -> list[Path]:
    ...
    if props and not props_mod.wanted(ctx.config, ctx.root):
        props = []
...
class PoseStage(Stage):
    ...
    def _from_llm(self, ctx: Context, cfg: dict, n_frames: int) -> list[dict]:
        ...
        for i, f in enumerate(frames):
            ...
            if issues:
                raise RuntimeError(
                    # not-a-message: generate_pose() returns only frames that already
                    # passed this check, so a failure means one was mutated after.
